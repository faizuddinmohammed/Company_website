...
function get_service_category_name_by_id($id)
{
    $return_val = __('uncategorized');

    $category_details = \App\ServiceCategory::find($id);
    if (!empty($category_details)) {
        $return_val = $category_details->name;
    }
            ...
            if (!empty($item['variant'])){
                foreach(json_decode($item['variant']) as $variant){
                    ...
                    if(!empty($variant_title)){
                        $output .= '<div class="product-variant-list-wrapper"><h5 class="title">'.$variant_title.'</h5><ul class="product-variant-list">';
                            $output .= '<li>'.$variant->term.'</li>';
                        $output .= '</ul></div>';
                    }
                }
            }
