...
class FrontendController extends Controller
{
    ...
    public function index()
    {
        ...
        $all_work_category = WorksCategory::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'desc')->take(get_static_option('home_page_01_service_area_items'))->get();
    ...
    public function home_page_change($id)
    {
        ...
        if(!in_array($id,['01','02','03','04','05','06','07','08','09','10','11','12','13','14','15','16','17','18'])){
            abort(404);
        }
        $lang = LanguageHelper::user_lang_slug();
        $all_header_slider = HeaderSlider::where('lang', $lang)->get();
    ...
    public function services_single_page($slug)
    {
        ...
        $service_details = ServiceDetail::where('service_id', $service_item->id)->get();
        //dd($service_item);
        return view('frontend.pages.service.service-single')->with(['service_item' => $service_item, 'service_category' => $service_category, 'price_plan' => $price_plan,'service_details' => $service_details]);
    }
    ...
    public function work_single_page($slug)
    {
        $default_lang = Language::where('default', 1)->first();
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
        $service_item = Works::where('slug', $slug)->first();
        if (empty($service_item)){
            abort(404);
        }
        $service_category = WorksCategory::where(['status' => 'publish', 'lang' => $lang])->get();
        $price_plan = !empty($service_item) && !empty($service_item->price_plan) ? PricePlan::find(unserialize($service_item->price_plan)) : '';
        $service_details = SolutionDetail::where('service_id', $service_item->id)->get();
        //dd($service_item);
        return view('frontend.pages.solution.service-single')->with(['service_item' => $service_item, 'service_category' => $service_category, 'price_plan' => $price_plan,'service_details' => $service_details]);
    }
    ...
    public function service_page()
    {
        ...
        $all_service_category = ServiceCategory::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'asc')->take(get_static_option('home_page_01_service_area_items'))->get();

        return view('frontend.pages.service.services')->with(['all_services' => $all_services, 'all_service_category' => $all_service_category]);
    }
    ...
    public function work_page()
    {
        $default_lang = Language::where('default', 1)->first();
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
        $all_services = Works::where('lang', $lang)->orderBy('sr_order', 'asc')->paginate(get_static_option('service_page_service_items'));
        $all_service_category = WorksCategory::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'asc')->take(get_static_option('home_page_01_service_area_items'))->get();

        return view('frontend.pages.solution.services')->with(['all_services' => $all_services, 'all_service_category' => $all_service_category]);
    }
    ...
    public function category_wise_works_page($id, $any)
    {
        $default_lang = Language::where('default', 1)->first();
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
        $category_name = WorksCategory::find($id)->name;
        if(empty($category_name)){
            abort('404');
        }
        $service_item = Works::where(['categories_id' => $id, 'lang' => $lang])->paginate(6);
        return view('frontend.pages.solution.service-category')->with(['service_items' => $service_item, 'category_name' => $category_name]);
    }
